...
import { CreditCard, Coins, Check, X, Zap, Crown, Monitor, Globe, ChevronLeft, ChevronRight, ChevronDown, Sparkles, FileText, FolderOpen } from 'lucide-react';
...
export default function Pricing() {
  ...
  return (
    <section id="pricing" className="py-16 md:py-24 bg-zinc-50 overflow-hidden">
      <div className="max-w-[1200px] mx-auto px-4 md:px-6">
        <div className="text-center mb-8 md:mb-12">
          <span className="inline-block px-3 py-1 rounded-full border border-zinc-200 text-[10px] font-bold uppercase tracking-widest bg-white mb-3 md:mb-4 text-zinc-500">Pricing</span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight mb-3 md:mb-4">합리적인 요금제</h2>
          <p className="text-zinc-500 text-base md:text-lg">브랜드 성장 단계에 맞춰 최적의 플랜을 선택하세요.</p>
        </div>

        <div className="flex justify-center mb-8 md:mb-10">
          <div className="bg-white p-1 md:p-1.5 rounded-xl md:rounded-2xl border border-zinc-200 inline-flex shadow-sm">
            <button onClick={() => setPricingMode('credits')} className={`px-4 md:px-8 py-2 md:py-2.5 rounded-lg md:rounded-xl text-xs md:text-sm font-bold transition-all flex items-center gap-1.5 md:gap-2 ${pricingMode === 'credits' ? 'bg-zinc-900 text-white' : 'text-zinc-500 hover:text-black'}`}>
              <Coins size={14} /> 크레딧 충전
            </button>
            <button onClick={() => setPricingMode('subscription')} className={`px-4 md:px-8 py-2 md:py-2.5 rounded-lg md:rounded-xl text-xs md:text-sm font-bold transition-all flex items-center gap-1.5 md:gap-2 ${pricingMode === 'subscription' ? 'bg-zinc-900 text-white' : 'text-zinc-500 hover:text-black'}`}>
        ...
        {pricingMode === 'credits' ? (
          <>
            {/* 설치형 프로그램 설명 - 가로형 */}
            <div className="bg-gradient-to-r from-zinc-900 to-zinc-800 rounded-xl md:rounded-2xl p-4 md:p-6 mb-8 md:mb-12">
              ...
              {/* 상세 기능 펙처지기 */}
              {desktopDetailOpen && (
                <div className="mt-4 pt-4 border-t border-white/10 grid grid-cols-2 md:grid-cols-6 gap-2 text-[11px]">
            ...
            {/* 3D 캐러셀 슬라이드 */}
            <div className="relative h-[480px] md:h-[520px] mb-6 md:mb-8" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} ref={sliderRef}>
                ...
                {CREDIT_PACKAGES.map((pack, idx) => {
                  ...
                  return (
                    <div key={idx} className="absolute w-[280px] md:w-[320px] transition-all duration-500 ease-out cursor-pointer" style={style} onClick={() => setCurrentSlide(idx)}>
                      <div className={`p-6 md:p-8 rounded-2xl md:rounded-3xl flex flex-col relative ${pack.popular && isCenter ? 'bg-zinc-900 text-white shadow-2xl' : isBest && isCenter ? 'bg-gradient-to-br from-purple-600 to-purple-800 text-white shadow-2xl' : isCenter ? 'bg-white border-2 border-zinc-900 shadow-2xl' : 'bg-white border border-zinc-200 shadow-lg'}`}>
                        ...
                        <div className="text-center mb-4">
                          ...
                          <p className={`text-xs md:text-sm ${(pack.popular || isBest) && isCenter ? 'text-zinc-300' : 'text-zinc-500'}`}>{formatPrice(pack.credits)} 크레딧</p>
                          <p className={`text-[10px] md:text-xs mt-1 ${(pack.popular || isBest) && isCenter ? 'text-zinc-400' : 'text-zinc-400'}`}>크레딧당 ₩{pack.pricePerCredit}{pack.discount > 0 && (<span className={`ml-1 font-bold ${(pack.popular || isBest) && isCenter ? 'text-[#87D039]' : 'text-red-500'}`}>({pack.discount}% 할인)</span>)}</p>
                        </div>
            ...
            <div className="flex justify-center gap-1.5 md:gap-2 mb-8 md:mb-10">
              {CREDIT_PACKAGES.map((_, idx) => (<button key={idx} onClick={() => setCurrentSlide(idx)} className={`h-1.5 md:h-2 rounded-full transition-all duration-300 ${currentSlide === idx ? 'bg-zinc-900 w-6 md:w-8' : 'bg-zinc-300 w-1.5 md:w-2'}`} />))}
            </div>
            ...
            <div className="bg-zinc-900 rounded-xl md:rounded-2xl p-4 md:p-6 flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3 md:gap-4">
                <div className="w-10 h-10 md:w-12 md:h-12 bg-[#87D039] rounded-lg md:rounded-xl flex items-center justify-center flex-shrink-0"><Monitor size={20} className="text-black" /></div>
                <div>
                  <h4 className="font-bold text-white text-sm md:text-base">크레딧 충전 후 설치형 프로그램 이용 가능</h4>
                  <p className="text-xs md:text-sm text-zinc-400">대량 일괄 처리 · 상품명 정제 · SEO 자동 생성 · 엑셀 출력</p>
                </div>
              </div>
              <button onClick={() => router.push('/mypage/api-keys')} className="w-full md:w-auto whitespace-nowrap px-5 md:px-6 py-2.5 md:py-3 bg-white text-black rounded-lg md:rounded-xl font-bold text-sm hover:bg-zinc-100 transition-colors">API 키 발급받기</button>
            </div>
          </>
        ) : (
          <>
            {/* 웹 버전 설명 - 가로형 */}
            <div className="bg-gradient-to-r from-blue-600 to-blue-500 rounded-xl md:rounded-2xl p-4 md:p-6 mb-8 md:mb-12">
              <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8">
                {/* 아이콘 + 제목 */}
                <div className="flex items-center gap-3 md:border-r md:border-white/20 md:pr-8">
            ...
            <div className="flex items-center justify-center gap-3 md:gap-4 mb-8 md:mb-10">
              <span className={`text-xs md:text-sm font-medium ${!isAnnual ? 'text-zinc-900' : 'text-zinc-400'}`}>월간</span>
              <button onClick={() => setIsAnnual(!isAnnual)} className={`w-10 md:w-12 h-6 md:h-7 rounded-full p-1 transition-colors duration-300 ${isAnnual ? 'bg-[#87D039]' : 'bg-zinc-300'}`}><div className={`w-4 md:w-5 h-4 md:h-5 bg-white rounded-full shadow-md transform transition-transform duration-300 ${isAnnual ? 'translate-x-4 md:translate-x-5' : 'translate-x-0'}`}></div></button>
              <span className={`text-xs md:text-sm font-medium flex items-center gap-1.5 md:gap-2 ${isAnnual ? 'text-zinc-900' : 'text-zinc-400'}`}>연간 <span className="text-[#87D039] text-[10px] md:text-xs font-bold">20% 할인</span></span>
            </div>
            ...
            <div className="md:hidden relative h-[420px] mb-6">
              <div className="absolute inset-0 flex items-center justify-center">
                {[{ id: 'free', name: 'Free', desc: '체험을 위한 무료 플랜', price: 0, credits: '5 크레딧 (1회 지급)', features: [{ text: '웹에서 결과 미리보기', included: true }, { text: 'Standard / Premium 모델 사용', included: true }, { text: '설치형 프로그램', included: false }, { text: '우선 처리', included: false }], buttonText: '무료로 시작', recommended: false }, { id: 'starter', name: 'Starter', desc: '정기 사용을 위한 구독', price: isAnnual ? 23200 : 29000, credits: '월 100 크레딧 (매월 갱신)', features: [{ text: '웹에서 결과 미리보기', included: true }, { text: 'Standard / Premium 모델 사용', included: true }, { text: '우선 처리', included: true }, { text: '설치형 프로그램', included: false }], buttonText: '구독 시작하기', recommended: true }].map((plan, idx) => {
                  ...
                  let style: React.CSSProperties;
                  if (isCenter) style = { transform: 'translateX(0) scale(1)', opacity: 1, zIndex: 30 };
                  else style = { transform: 'translateX(-70%) scale(0.85)', opacity: 0.5, zIndex: 20 };
                  return (
                    <div key={plan.id} className="absolute w-[280px] transition-all duration-500 ease-out cursor-pointer" style={style}>
                      <div className={`p-6 rounded-2xl flex flex-col relative ${plan.recommended ? 'bg-gradient-to-br from-blue-600 to-blue-500 text-white shadow-2xl' : 'bg-white border-2 border-zinc-200 shadow-lg'}`}>
                        ...
                        <div className="text-center mb-4 mt-2">
                          ...
                          <p className={`text-xs ${plan.recommended ? 'text-blue-100' : 'text-zinc-500'}`}>{plan.desc}</p>
                        </div>
                        <div className="text-center mb-4">
                          <div className="text-3xl font-bold mb-1">₩{formatPrice(plan.price)}{plan.price > 0 && <span className={`text-sm font-normal ${plan.recommended ? 'text-blue-100' : 'text-zinc-400'}`}>/월</span>}</div>
                          <p className={`text-xs ${plan.recommended ? 'text-blue-100' : 'text-zinc-500'}`}>{plan.credits}</p>
                          {isAnnual && plan.price > 0 && <p className="text-[10px] text-[#87D039] mt-1">연간 결제 시 20% 할인</p>}
                        </div>
                        <div className={`rounded-xl p-3 mb-4 flex-1 ${plan.recommended ? 'bg-white/10' : 'bg-zinc-50'}`}>
                        ...
                        <button onClick={() => handleSubscribe(plan.id)} className={`w-full py-3 rounded-xl font-bold text-sm transition-all ${plan.recommended ? 'bg-white text-blue-600 hover:bg-blue-50' : 'bg-zinc-100 text-zinc-700 hover:bg-zinc-200'}`}>{plan.buttonText}</button>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="hidden md:grid grid-cols-2 gap-6 max-w-[700px] mx-auto mb-8 md:mb-10">
              <div className="p-6 md:p-8 rounded-2xl md:rounded-3xl flex flex-col bg-white border-2 border-zinc-200 shadow-lg hover:border-zinc-400 transition-all">
                <div className="text-center mb-4 md:mb-6"><h3 className="text-lg md:text-xl font-bold mb-1">Free</h3><p className="text-xs md:text-sm text-zinc-500">체험을 위한 무료 플랜</p></div>
                <div className="text-center mb-4 md:mb-6"><div className="text-3xl md:text-4xl font-bold mb-1">₩0</div><p className="text-xs md:text-sm text-zinc-500">5 크레딧 (1회 지급)</p></div>
                ...
                <button onClick={() => handleSubscribe('free')} className="w-full py-3 md:py-3.5 rounded-xl font-bold text-sm transition-all bg-zinc-100 text-zinc-700 hover:bg-zinc-200">무료로 시작</button>
              </div>
              <div className="p-6 md:p-8 rounded-2xl md:rounded-3xl flex flex-col bg-gradient-to-br from-blue-600 to-blue-500 text-white shadow-2xl relative">
                <div className="absolute -top-3 left-1/2 -translate-x-1/2"><span className="bg-[#87D039] text-black text-[10px] font-bold px-4 py-1 rounded-full">추천</span></div>
                <div className="text-center mb-4 md:mb-6 mt-2"><h3 className="text-lg md:text-xl font-bold mb-1">Starter</h3><p className="text-xs md:text-sm text-blue-100">정기 사용을 위한 구독</p></div>
                <div className="text-center mb-4 md:mb-6"><div className="text-3xl md:text-4xl font-bold mb-1">₩{formatPrice(isAnnual ? 23200 : 29000)}<span className="text-sm md:text-base font-normal text-blue-100">/월</span></div><p className="text-xs md:text-sm text-blue-100">월 100 크레딧 (매월 갱신)</p>{isAnnual && <p className="text-[10px] text-[#87D039] mt-1">연간 결제 시 20% 할인</p>}</div>
                ...
                <button onClick={() => handleSubscribe('starter')} className="w-full py-3 md:py-3.5 rounded-xl font-bold text-sm transition-all bg-white text-blue-600 hover:bg-blue-50">구독 시작하기</button>
              </div>
            </div>
        ...
        <div className="mt-8 md:mt-10 bg-white rounded-xl md:rounded-2xl border border-zinc-200 p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6">
          <div className="text-center md:text-left"><h3 className="text-lg md:text-xl font-bold mb-2">Enterprise</h3><p className="text-zinc-500 text-xs md:text-sm">월 1,000건 이상 대량 처리가 필요한 기업을 위한 맞춤 플랜</p></div>
          <button onClick={() => toast('문의 기능은 준비 중입니다', { icon: '📧' })} className="w-full md:w-auto whitespace-nowrap px-5 md:px-6 py-2.5 md:py-3 bg-zinc-900 text-white rounded-lg md:rounded-xl font-bold text-sm hover:bg-black transition-colors">문의하기</button>
        </div>
      </div>
    </section>
  );
