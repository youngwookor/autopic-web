...
app = FastAPI(
    title="Autopic API",
    description="AI 상품 이미지 생성 + 결제 API",
    version="1.0.0"
)
    ...
    try:
        ...
        if result.data:
            ...
            if data.get('success'):
                ...
                raise HTTPException(status_code=400, detail=data.get('error', '크레딧 차감 실패'))
    ...
    try:
        ...
        if result.data:
            ...
            if data.get('success'):
                ...
                raise HTTPException(status_code=400, detail=data.get('error', '크레딧 추가 실패'))
...
async def save_generation(user_id: str, image_urls: List[str], mode: str, model_type: str, credits_used: int):
    """생성 내역 저장"""
    try:
        for url in image_urls:
            supabase.table("generations").insert({
                ...
                "credits_used": credits_used // len(image_urls) if image_urls else credits_used,
            }).execute()
    except Exception as e:
...
async def confirm_payment(request: PaymentConfirmRequest):
    ...
    try:
        ...
        payment_result = supabase.table("payments").select("*").eq("order_id", request.order_id).single().execute()
...
async def generate_user_api_key(request: APIKeyRequest):
    ...
    try:
        # 기존 키 확인 (활성 키 제한: 3개)
        existing = supabase.table("api_keys").select("*").eq("user_id", request.user_id).eq("is_active", True).execute()
...
async def get_user_api_keys(user_id: str):
    """사용자 API 키 목록 조회"""
    try:
        result = supabase.table("api_keys").select("id, name, key_hash, is_active, created_at, last_used_at").eq("user_id", user_id).execute()
...
async def verify_api_key(api_key: str) -> Optional[str]:
    ...
    try:
        key_hash = hash_api_key(api_key)
        result = supabase.table("api_keys").select("user_id").eq("key_hash", key_hash).eq("is_active", True).single().execute()
        
        if result.data:
            # 마지막 사용 시간 업데이트
            supabase.table("api_keys").update({"last_used_at": datetime.now().isoformat()}).eq("key_hash", key_hash).execute()
...
async def desktop_generate_image(
    request: DesktopGenerateRequest,
    ...
    if not rate_limiter.is_allowed(rate_key, RATE_LIMITS["generate"]["limit"], RATE_LIMITS["generate"]["window"]):
        remaining = rate_limiter.get_remaining(rate_key, RATE_LIMITS["generate"]["limit"])
        raise HTTPException(
            status_code=429, 
...
async def desktop_get_credits(x_api_key: str = Header(None, alias="X-API-Key")):
    ...
    if not rate_limiter.is_allowed(rate_key, RATE_LIMITS["api_key"]["limit"], RATE_LIMITS["api_key"]["window"]):
        raise HTTPException(status_code=429, detail="요청 횟수 초과. 1분 후 다시 시도해주세요.")
